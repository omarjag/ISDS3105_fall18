...
```{r}
tbl(con, sql('SELECT hotelName FROM review, hotel WHERE hotel.hotelId = review.hotelId GROUP BY review.hotelId ORDER BY AVG(reviewService) LIMIT 3'))
```
...
```{r}
tbl(con, sql('select authorNickname, h.hotelId, hotelName
            from hotel h join
            review r join
            author a
